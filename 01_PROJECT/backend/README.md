...
Full implementation report: [`02_DOCUMENTATION/05_BACKEND/BACKEND_IMPLEMENTATION_REPORT.md`](../../02_DOCUMENTATION/05_BACKEND/BACKEND_IMPLEMENTATION_REPORT.md)
...
```bash
python tasks.py build-db                     # produce 01_PROJECT/backend/data/
...
**The frozen model** — see `02_DOCUMENTATION/02_MODEL/MODEL_FREEZE.md`:
...
Interactive documentation at `/docs`; schema at `01_PROJECT/backend/openapi.json`
...
```bash
# 01_PROJECT/backend/.env — never committed, never baked into an image
...
uvicorn from `01_PROJECT/backend/` — so `01_PROJECT/backend/.env` is the file that counts. Both
...
[`02_DOCUMENTATION/07_GENAI/GENAI_IMPLEMENTATION_REPORT.md`](../../02_DOCUMENTATION/07_GENAI/GENAI_IMPLEMENTATION_REPORT.md).
...
* offer price what-if simulation. The frozen model's measured response to
  ...
  See `02_DOCUMENTATION/04_ARCHITECTURE/PRODUCT_ARCHITECTURE_PLAN.md` §14.3.
