...
API needs into 01_PROJECT/backend/data/.
...
01_PROJECT/backend/data/. This script never imports `pipeline` — importing
...
# scripts/ -> backend -> 01_PROJECT -> repository root
REPO_ROOT = Path(__file__).resolve().parents[3]
BACKEND = Path(__file__).resolve().parents[1]
# Every input below is a PROTECTED research artefact, opened read-only. They live
# under 03_RESEARCH/ as one unit because the research pipeline derives its own
# root from `pipeline/`'s parent and needs these as siblings.
ROOT = REPO_ROOT / "03_RESEARCH"
DOCS = REPO_ROOT / "02_DOCUMENTATION"
