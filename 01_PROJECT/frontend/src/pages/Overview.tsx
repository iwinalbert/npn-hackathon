        ...
        <Async query={prov} height="h-40">
          {(p) => (
            <Card title="What is behind these numbers" subtitle="Every figure traces to a stored artefact">
              ...
              <Explain>
                The deployed model files are hash-checked on every request to the model
                page, and the served forecast is verified against the frozen artefact.
                You can re-run the model yourself from the{' '}
                <Link to="/model" className="text-forecast underline underline-offset-2">
                  Model
                </Link>{' '}
                page.
              </Explain>
      ...
      <Card title="Start here">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { to: '/validation', t: 'Check it against reality', d: 'Replay a historical window where the true outcome is known.' },
            { to: '/methodology', t: 'Read the limits', d: 'What the data cannot support, and what the research rejected.' },
          ].map((c) => (
            <Link
              key={c.to}
              to={c.to}
