...
`01_PROJECT/backend/README.md` that running the recursive member "takes minutes". That
...
`01_PROJECT/backend/` and `01_PROJECT/frontend/` contain only a README and `.gitkeep`. There is
...
| **Build script** | One-time materialisation of `product.duckdb` from protected artefacts | Writing anywhere outside `01_PROJECT/backend/data/` |
...
```
protected artefacts (read-only) → 01_PROJECT/backend/scripts/build_product_db.py
                                → 01_PROJECT/backend/data/product.duckdb
```
...
1. **Direct parquet attachment.** DuckDB queries
   ...
   data.
2. **`01_PROJECT/backend/data/product.duckdb`** — a small generated database holding only
...
| Research-tree integrity | DuckDB opened `read_only=True` on the parquet; the API process never writes outside `01_PROJECT/backend/data/` |
...
| File | Purpose |
|---|---|
| `01_PROJECT/backend/scripts/build_product_db.py` | Read protected artefacts read-only → write `data/product.duckdb`. Builds all 6 tables; computes error bands from backtest residuals; embeds hashes |
| `01_PROJECT/backend/requirements.txt` | Pin fastapi, uvicorn, duckdb, pydantic-settings, pytest, httpx (+ lightgbm/pandas/numpy for the worker) |
| `01_PROJECT/backend/app/config.py` | Settings: artefact paths, DB path, CORS origins, cache TTL |
| `01_PROJECT/backend/app/db.py` | DuckDB connection factory, read-only parquet attach, identifier whitelist |
| `01_PROJECT/frontend/package.json`, `vite.config.ts`, `tailwind.config.js` | Scaffold + `/api` proxy |
...
`docker-compose.yml` · `tests/e2e/` · `01_PROJECT/backend/README.md` (corrected) ·
`02_DOCUMENTATION/DEMO_SCRIPT.md`
...
**Product code writes only to** `01_PROJECT/backend/`, `01_PROJECT/frontend/`, and
`02_DOCUMENTATION/`.

Enforced three ways: read-only container mount; `read_only=True` DuckDB handles;
and a CI integrity check reusing
`03_RESEARCH/scripts/08_organization/61_integrity_manifest.py`.
