...
|---|---|---|
| `NPN_DATA_DIR` | `01_PROJECT/backend/data` | the three product artefacts |
...
### Docker
```bash
docker build -f 01_PROJECT/backend/Dockerfile --target full -t npn-api .
docker compose up
```
...
Everything the backend wrote went to `01_PROJECT/backend/` and `02_DOCUMENTATION/`.
