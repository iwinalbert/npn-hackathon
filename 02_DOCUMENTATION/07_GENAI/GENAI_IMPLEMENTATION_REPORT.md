...
| File | Role |
|---|---|
| `01_PROJECT/backend/app/services/genai_context.py` | Context builder — intent routing, deterministic statistics |
| `01_PROJECT/backend/app/services/genai.py` | Provider abstraction, system instruction, guardrails, grounding |
| `01_PROJECT/backend/app/routers/genai.py` | 4 endpoints |
| `01_PROJECT/backend/tests/test_genai.py` | 69 offline tests |
| `01_PROJECT/backend/tests/test_genai_live.py` | 6 live tests (opt-in, `-m live`) |
| `01_PROJECT/frontend/src/pages/Assistant.tsx` | The UI |
| `01_PROJECT/frontend/src/test/assistant.test.tsx` | 15 tests |
...
```bash
# Local development — 01_PROJECT/backend/.env
...
`01_PROJECT/backend/.env` is the location that matters: `tasks.py api` launches uvicorn
...
| Where | Name | Works |
|---|---|---|
| `01_PROJECT/backend/.env` | `GEMINI_API_KEY` | ✅ |
| `01_PROJECT/backend/.env` | `NPN_GEMINI_API_KEY` | ✅ |
...
**522** files recorded in `02_DOCUMENTATION/09_VALIDATION/_integrity/manifest_after.json`:
**0 deleted, 0 modified**. All GenAI work is confined to
`01_PROJECT/backend/app/{services,routers}/genai*`, `01_PROJECT/frontend/src/pages/Assistant.tsx`,
...
Tests constructed `Settings()` directly, which reads `01_PROJECT/backend/.env`. With a
