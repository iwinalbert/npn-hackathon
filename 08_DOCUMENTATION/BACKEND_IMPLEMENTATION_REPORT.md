...
|---|---|---|
| `NPN_DATA_DIR` | `06_BACKEND/data` | the three product artefacts |
...
### Docker
```bash
docker build -f 06_BACKEND/Dockerfile --target full -t npn-api .
docker compose up
```
...
Everything the backend wrote went to `06_BACKEND/` and `08_DOCUMENTATION/`.
