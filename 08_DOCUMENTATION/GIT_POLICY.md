...
ARE committed** (`08_DOCUMENTATION/_integrity/`), so a reviewer can verify that
...
| **Integrity manifests** | `08_DOCUMENTATION/_integrity/` — SHA-256 of 522 files | ~400 KB |
| Product code | `06_BACKEND/`, `07_FRONTEND/` | grows |
...
| `02_MODEL/FROZEN_CHAMPION/*.txt` | 29 MB | Copies of the two frozen members | SHA-256 committed in `CHAMPION_MANIFEST.json` |
...
| `03_FORECASTS/`, `09_SUBMISSION/` CSVs | 34 MB | Copies of the above | Copies of a hash-verified source |
| `MY_RESEARCH_PAPER/reproduction/*.csv` | 34 MB | Champion validation predictions | Regenerate via `audit_reproduce.py` |
| `06_BACKEND/data/*.duckdb` | varies | **Product-generated**, fully rebuildable | `make build-db` |
...
mechanism is **Git LFS** for `02_MODEL/FROZEN_CHAMPION/*.txt` (29 MB, two files)
