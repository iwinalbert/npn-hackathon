"""
Render the system architecture diagram to PNG.

    python make_arch.py

Hand-laid-out rather than auto-routed: an architecture drawing is read, not
generated, and graph layout engines put the boxes where the algorithm likes
rather than where the reader needs them.

Box heights are COMPUTED from their content (see `bh`). The first draft passed
them in by hand and the last line of six different boxes rendered on top of its
own bottom border.
"""
...
ACCENT    = "#0E7A85"          # the deployed unit
ACCENT_BG = "#E7F3F4"
FROZEN    = "#9C6526"          # frozen / read-only artefacts
FROZEN_BG = "#F8F0E5"
GHOST     = "#8B98A5"          # outside the default deployment
GHOST_BG  = "#F4F6F8"

SANS = "DejaVu Sans"
MONO = "DejaVu Sans Mono"

# vertical metrics, in axis units
TITLE_DROP = 3.2      # title centre below the box top
LINE_1     = 3.9      # first body line below the title
LINE_GAP   = 2.9
PAD_BOT    = 2.7


def bh(n_lines: int) -> float:
    """Height a box needs for `n_lines` of body text."""
    if n_lines == 0:
        return 6.2
...
def box(x, y, w, title, lines=(), *, edge=ACCENT, face=ACCENT_BG,
    ...
    """A node. Height is derived from the content, never passed in."""
    lines = list(lines)
    h = bh(len(lines))
    ...
    # The badge shares the TITLE row: in the first draft it sat in the bottom
    # padding and collided with the last body line.
    if tag:
...
def zone(x, y, w, h, label, *, edge=RULE, lc=MUTED):
    """A dashed boundary with the label tucked into the top edge."""
    ax.add_patch(FancyBboxPatch(
        (x, y), w, h,
...
# read-only data mount, directly below the api
...
# the space beside the mount carries the claim that matters most
...
# ===========================================================================
# External egress  (drawn outside the host boundary, on purpose)
# ===========================================================================
...
# Sits BELOW the last box (bottom edge 15.2). Placed at 22.5 in the previous
# draft, where it ran straight through the /data/product row.
ax.text(120, 12.0,
